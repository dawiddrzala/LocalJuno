...
STAKE=${STAKE_TOKEN:-ujunox}
FEE=${FEE_TOKEN:-ucosm}
CHAIN_ID=${CHAIN_ID:-testing}
MONIKER=${MONIKER:-node001}
KEYRING="--keyring-backend test"
VALIDATORMNEMONIC="satisfy adjust timber high purchase tuition stool faith fine install that you unaware feed domain license impose boss human eager hat rent enjoy dawn"
...
if ! junod keys show validator $KEYRING; then
  (echo "$VALIDATORMNEMONIC" ; echo "$PASSWORD"; echo "$PASSWORD") | junod keys add validator --recover $KEYRING
